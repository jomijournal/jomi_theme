...
<tr>
  <th>Require Login?</th>
  <td>
    <select id="inst-order-require-login">
      <?php if($order->require_login === 0 || $order->require_login === "F" || $order->require_login === "0") { ?>
        <option val="T">T</option>
        <option val="F" selected>F</option>
      <?php } else { ?>
        <option val="T" selected>T</option>
